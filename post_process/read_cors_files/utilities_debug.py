...
def angle_and_p2(px, py, pz, zenith_angle, masses, pdgs, obs_levels):
    ...
    with np.errstate(all='raise'):
        try:
            ...
            res = np.sum(ndir * ndir0, axis=1)
            print('oh no!:')
            ids = np.where(abs(res) > 1)[0]
            print(f"num of failed ids = {len(ids)} from {len(px)}")
            max_id = min(len(ids)-1, 30)
            print(res[ids[0:max_id]])
            
            print(ndir0)
            print(ndir[ids[0:max_id]])
            print(p[ids[0:max_id]])
            print(f"Pdgs = {pdgs[ids[0:max_id]]}")
            print(f"Obs_lev = {obs_levels[ids[0:max_id]]}")
            etot = np.sqrt(p2 + masses**2)
            ekin = p2/(np.sqrt(p2 + masses**2) + masses)
            print(f"Etot = {etot[ids[0:max_id]]}")
            print(f"Ekin = {ekin[ids[0:max_id]]}")
            # print("\nAngles:")
            # print(angles[ids[0:max_id]])
...
def mass_pdg_level(particle_description):
    ...
    """
    pdg_map, mass_map = pdg_mass_maps()
...
def final_values(particle_description, px, py, pz, zenith_angle):
    ...
    angles, p2 = angle_and_p2(px, py, pz, zenith_angle, masses, pdgs, obs_levels)
